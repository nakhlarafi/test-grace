...
for i in tqdm(range(int(len(lst) / totalnum) + 1)):
    ...
    for j in range(totalnum):
        ...
        p = subprocess.Popen("CUDA_VISIBLE_DEVICES=0,1 torchrun --nnodes=1 --nproc_per_node=3 --rdzv_id=100 --rdzv_backend=c10d --rdzv_endpoint=$MASTER_ADDR:virya4:29405 run.py %d %s %f %d %d"%(lst[totalnum * i + j], project, lr, seed, batch_size), shell=True)
        jobs.append(p)
        time.sleep(10)
    for p in jobs:
        p.wait()
